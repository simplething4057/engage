...
10. [예상 비용](#10예상-비용)
11. [성공 지표 (KPI)](#11성공-지표-kpi)
12. [개발 로드맵](#12개발-로드맵)
13. [보완이 필요한 사항](#13보완이-필요한-사항)
14. [라이선스](#14라이선스)
...
아맘때 서비스는 AI 기반 비대면 발달심리 검사와 상담사 전화 상담을 제공합니다. 검사 완료 후 해석 상담까지 2~3일의 공백이 존재하며, 이 기간에 보호자는 임상 용어가 포함된 보고서를 혼자 읽으며 불안이 심화되거나 참여도 저하로 이어질 수 있습니다. 이는 상담 이탈, CS 문의 증가, 서비스 신뢰도 저하로 이어집니다.
...
| 변수명 | 값 | 설명 |
|--------|-----|------|
| `OLLAMA_ORIGINS` | `*` | Ollama CORS 허용 — 브라우저에서 localhost:11434 접근 시 필수 |
| `VITE_APP_PORT` | `5173` (기본값) | Vite 개발 서버 포트 (선택) |
...
> export OLLAMA_ORIGINS="*"             # Linux (세션 임시)
> ```
...
AI는 보호자의 불안을 완화하는 데 집중하며, 임상 판단은 상담사 고유 영역으로 엄격히 분리합니다.

| AI 수행 영역 | AI 비수행 영역 (상담사 전담) |
|-------------|---------------------------|
| CBCL 용어 설명 (T점수·내재화·외현화) | 개인 수치의 임상적 의미 해석 |
| 상담 절차·소요 시간 안내 | 진단 또는 예후 언급 |
| 보호자 감정에 공감 응답 | 전문 기관 연계 결정 |

**구현 방법:** 시스템 프롬프트로 응답 범위를 하드코딩 — 결과 해석 요청 시 `"정확한 해석은 상담사와 함께 확인하시는 게 좋아요."` 고정 응답만 출력합니다.

**데이터 보안:** 모든 AI 추론이 로컬에서만 처리됩니다. 보호자의 입력 내용은 외부 서버로 전송되지 않습니다.
...
| 항목 | 비용 |
|------|------|
| AI API 호출 | **$0** — Ollama 로컬 실행, 외부 API 없음 |
| 인프라 | **$0** — 개발자 로컬 머신 활용 |
| 모델 라이선스 | **무료** — EXAONE 3.5는 연구·상업 이용 허용 |

### 본 서비스 전환 시 (월 1,000명 기준, 대화당 평균 2,000 tokens)

| 시나리오 | 월 토큰 | 월 예상 비용 |
|----------|---------|-------------|
| 로컬 GPU 서버 (EXAONE 7.8B) | 2M | GPU 서버비 $50~200, 토큰 비용 $0 |
| GPT-4o mini | 2M | 약 $0.30 |
| Claude Sonnet | 2M | 약 $6 |

> 서비스 규모와 데이터 보안 요구 수준에 따라 로컬 LLM 유지 또는 클라우드 API 전환을 결정합니다. 아동 심리 민감 정보 특성상 로컬 추론 유지를 우선 권장합니다.
...
- [x] K-CBCL 실제 보고서 기반 시스템 프롬프트 용어 사전 (위축·우울/불안·주의집중 문제 등 8개 증후군 포함)
- [x] 보고서 기반 빠른 질문 5개 (준임상 범위, ADHD 오해, 집·학교 행동 차이 등)
- [x] 보고서 기반 사전 질문 수집 항목 개선
- [x] 사전 질문 수집 폼
- [x] .ics 캘린더 연동
- [x] 리마인더 플로우 + CS 연결
- [x] 반응형 레이아웃
...
### 2차 개발
- [ ] **보고서 파싱 + AI 개인화**: 보호자의 검사 결과 PDF를 파싱하여 실제 수치(T점수·백분위)를 AI 채팅 컨텍스트에 주입. "이 아이의 내재화 점수(61T)는 준임상 범위예요"처럼 개인화된 응답 제공.
- [ ] **브리징 메시지 분기**: 점수 패턴(전체 정상 / 내재화만 준임상 / 임상 범위 척도 있음)에 따라 브리징 메시지 자동 분기. 불안 수준에 맞는 맞춤 안내 제공.
- [ ] 상담사 대시보드 (사전 질문 요약 뷰)
